...
test("calculateSuggestedPrice sem taxas nem anúncio: preço = custo * (1 + margem)", () => {
  const result = calculateSuggestedPrice({ baseCost: 10, feeTable: NO_FEE_TABLE, marginPercent: 30, roas: 0 });
...
test("calculateSuggestedPrice com margem 0% cobre exatamente o custo", () => {
  const result = calculateSuggestedPrice({ baseCost: 50, feeTable: NO_FEE_TABLE, marginPercent: 0, roas: 0 });
  assert.ok(Math.abs(result.price - 50) < 1e-9);
  assert.ok(Math.abs(result.profit - 0) < 1e-9);
});

test("calculateSuggestedPrice permite margem acima de 100% (markup sobre custo, não trava)", () => {
  const result = calculateSuggestedPrice({ baseCost: 50, feeTable: NO_FEE_TABLE, marginPercent: 200, roas: 0 });
  assert.equal(result.error, null);
...
test("calculateSuggestedPrice retorna erro quando taxa de marketplace + taxa de anúncio >= 100%, independente da margem", () => {
  ...
  const result = calculateSuggestedPrice({ baseCost: 50, feeTable, marginPercent: 30, roas: 1.5 }); // 50% taxa + 66% anúncio
  assert.ok(result.error);
  assert.equal(result.price, 0);
});

test("calculateSuggestedPrice considera ROAS como taxa de anúncio (1/ROAS)", () => {
  const semAnuncio = calculateSuggestedPrice({ baseCost: 20, feeTable: NO_FEE_TABLE, marginPercent: 20, roas: 0 });
  const comAnuncio = calculateSuggestedPrice({ baseCost: 20, feeTable: NO_FEE_TABLE, marginPercent: 20, roas: 4 });
...
test("calculateSuggestedPrice resolve corretamente a faixa de taxa por preço (tiers)", () => {
  ...
  const barato = calculateSuggestedPrice({ baseCost: 10, feeTable, marginPercent: 10, roas: 0 });
  assert.equal(barato.feeFixed, 4);
  assert.ok(barato.price < 80);

  // custo alto -> deve cair na segunda faixa
  const caro = calculateSuggestedPrice({ baseCost: 200, feeTable, marginPercent: 10, roas: 0 });
  assert.equal(caro.feeFixed, 16);
  assert.ok(caro.price >= 80);
});

test("calculateSuggestedPrice: lucro reportado bate com preço - custo - taxa - anúncio, e com custo * margem", () => {
  const feeTable = { tiers: [{ min: 0, max: Infinity, pct: 0.14, fixed: 20 }] };
  const result = calculateSuggestedPrice({ baseCost: 30, feeTable, marginPercent: 25, roas: 5 });
...
test("calculateSuggestedPrice: caso real reportado pelo usuário (Shopee, custo 16,895, margem 80%)", () => {
  ...
  const result = calculateSuggestedPrice({ baseCost: 16.895, feeTable, marginPercent: 80, roas: 0 });
  assert.equal(result.error, null);
